...
public class SkipList<K extends Comparable<K>, V> {
    ...
    public void insert(K key, V element) {
        ...
        SkipNode<K, V>[] update = (SkipNode<K, V>[]) Array.newInstance(SkipNode.class, level + 1);
        SkipNode<K, V> x = head;
    ...
    private SkipNode<K, V> searchNode(K searchKey) {
        ...
        for (int i = level; i >= 0; i--)
        {
            while ((x.forward[i] != null) && (searchKey.compareTo(x.forward[i].key()) > 0))
            {
                x = x.forward[i];
            }
        }
    ...
    public SkipNode<K, V>[] dump() {
        ...
        while (temp != null) {
            if (length == 0)
            {
                list[0] = new SkipNode<K, V>(temp.key(), temp.element(),temp.forward.length);
                length++;
            }
            else
            {
